...
export const getWeather = async (province, city) => {
  if (
    ...
    !CITY_INFO[province][city]["AREAID"]
  ) {
...
export const getBirthdayMessage = () => {
  ...
  birthdayList.forEach((item) => {
    ...
    // 节日相关
    if (item.type === "节日") {
      if (!config.FESTIVALS_LIMIT || festivalsCount < config.FESTIVALS_LIMIT) {
        // 节日相关
        if (item.type === "节日") {
          if (item.diffDay === 0) {
...
export const sendMessageReply = async (
  ...
  users.forEach(async (user) => {
    allPormise.push(sendMessage(templateId, user, accessToken, params));
  });
...
export const callbackReply = async (templateId, users, accessToken, params) => {
  users.forEach(async (user) => {
    await sendMessage(templateId, user, accessToken, params);
  });
};
